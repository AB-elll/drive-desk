...
- [ ] 判定カテゴリ（初期）:
  ...
  - 法務: 契約書、定款、登記書類
  - その他: 上記に該当しないファイル
- [ ] 信頼度スコアを付与し、低信頼度の場合は `low_confidence` フラグを立てて処理は継続する
...
### 3.4 処理（Processor）
- [ ] **複数プロセッサーの並列実行**: 1つのファイルを複数の会計ソフトへ同時に処理できる
- [ ] **プロセッサータイプ**:
  - `freee`（API型）: freee API v2 経由でリアルタイム登録
  - `jdl_csv`（CSV型）: JDL IBEX インポート用CSVを自動生成・Drive出力
  - 将来: `mf`（マネーフォワード）、`yayoi`（弥生）等
- [ ] **登録モードはクライアント設定で制御**（API型のみ）
  - `finalize`（デフォルト）: 確定登録
  - `draft`: 下書き登録
- [ ] 1ファイルに複数取引が含まれる場合（カード明細等）は取引ごとに個別登録する
- [ ] `processor_refs` はプロセッサーごとにネストして保存する
...
| タイプ | 連携方法 | 処理タイミング |
|--------|---------|--------------|
| API型（freee等） | REST API | リアルタイム |
| CSV型（JDL等） | CSV生成→Drive出力 | 設定スケジュール（日次・週次等） |

**処理フロー**

| 状況 | 動作 |
|------|------|
| 正常に分類・抽出できた | 設定された全プロセッサーで処理 |
| 分類・抽出はできたが信頼度低 | 同上 + ログに低信頼フラグを記録 |
| 完全に処理不能 | ログに記録・バッチ通知（頻度は設定可能） |

> **設計方針**: DriveDesk 内に承認フローを持たない。信頼度に関わらず自動処理し、経理担当者の確認は各会計ソフト上の通常業務の中で完結させる。

**Kara'sユースケース（freee + JDL併用）**
...
| `processor_refs` | プロセッサーごとの登録参照ID（ネストJSON） | `{"freee":["txn_001"],"jdl_csv":"export_0401.csv"}` |
| `updated_at` | 最終更新日時 | - |
...
- [ ] 全処理をGoogleスプレッドシートに記録（人間用監査ログ）
  - 記録項目: ファイル名、shared_at、primary_date、分類結果、low_confidenceフラグ、処理結果、登録先ID
...
- 情報提供者: Kara's代表者
- 情報受領者: 経理担当コンサル（翔太さん）
- 会計ソフト: freee（導入予定・承認済み）+ JDL IBEX（月次CSV連携）
- 通知手段: Googleスプレッドシート（候補）

### 5.2 典型的なフロー
1. 代表者がレシート写真・請求書PDFをDriveにアップロード
2. DriveDesKが検知 → SQLiteに `shared_at` とともにレコード作成
3. AIで「レシート（経理）」と分類 → 信頼度スコアを記録（低信頼度は `low_confidence: true`）
4. OCRで金額・日付・店舗名を抽出 → `primary_date` / `dates` を確定
5. freee APIへ確定登録（信頼度に関わらず処理）
6. 月次バッチでJDL IBEX用CSVを生成 → 指定DriveフォルダへCSVを出力
7. SQLite・Driveプロパティを `processed` に更新、`processor_refs` に各登録IDを保存
8. スプレッドシートに監査ログを追記

### 5.3 処理不能ケース
処理不能ファイルは自動処理せず `unprocessable` としてログに記録し、日次バッチで通知する。

- 画像が不鮮明でOCR・分類が全くできない
- ファイルが破損・暗号化されていて読み取れない
- 対応していないファイル形式
